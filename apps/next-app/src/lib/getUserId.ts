...
export default async function getUserIdFromRequest(req: NextRequest) {
  ...
  if (!token) {
    return { error: NextResponse.json({ success: false, message: "User Unauthorized" }, { status: 401 }) };
  }

  const jwtSecret = process.env.JWT_SECRET;
  if (!jwtSecret) {
    return { error: NextResponse.json({ success: false, message: "JWT secret not set" }, { status: 500 }) };
  }
