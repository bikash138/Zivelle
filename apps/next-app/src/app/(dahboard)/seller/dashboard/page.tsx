    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, index) => (
          ...
          >
            <Card className={`hover:shadow-lg transition-shadow duration-200 border ${stat.borderColor}`}>
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-600">{stat.title}</p>
                    <p className="text-2xl font-bold text-gray-900 mt-1">{stat.value}</p>
                    <Badge variant="secondary" className={`mt-2 ${stat.color} ${stat.bgColor} border-0`}>
                      {stat.change}
                    </Badge>
                  </div>
                  ...
                  recentOrders.map((order, index) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4">
                        <Badge 
                          variant={
                            order.order.orderStatus === 'Delivered' ? 'default' : 
                            order.order.orderStatus === 'Shipped' ? 'secondary' : 'outline'
                          }
                          className={
                            order.order.orderStatus === 'Delivered' ? 'bg-green-100 text-green-800 border-0' :
                            order.order.orderStatus === 'Shipped' ? 'bg-blue-100 text-blue-800 border-0' :
